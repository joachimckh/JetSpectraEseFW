...
int main(int argc, char *argv[]) {
  ...
  TH1F* h_in_inclusive_raw_tmp = dynamic_cast<TH1F*>(f->Get("hv_0"));
  TH1F* h_out_inclusive_raw_tmp = dynamic_cast<TH1F*>(f->Get("hv_1"));
  ...
  auto hPtInclusive_tmp = h->Projection(1);

  auto hPtInclusive = hPtInclusive_tmp->Rebin(n, "hInInclusiveReb", bin_edges);
  auto h_in_inclusive_raw = h_in_inclusive_raw_tmp->Rebin(n, "h_in_inclusive_raw", bin_edges);
  auto h_out_inclusive_raw = h_out_inclusive_raw_tmp->Rebin(n, "h_out_inclusive_raw", bin_edges);
  ...
  hPtInclusive->SetMarkerStyle(33);
  hPtInclusive->SetMarkerColor(kBlack);
  ...
  h_in_inclusive_raw->SetLineColor(kRed+1);
  h_in_inclusive_raw->SetMarkerStyle(20);
  h_in_inclusive_raw->SetMarkerColor(kRed+1);
  h_in_inclusive_raw->Draw("same");

  h_out_inclusive_raw->SetLineColor(kBlue);
  h_out_inclusive_raw->SetMarkerStyle(21);
  h_out_inclusive_raw->SetMarkerColor(kBlue);
  ...
  hRatioOut->SetMarkerStyle(21);
  hRatioOut->SetMarkerColor(kBlue);
  hRatioOut->Draw();

  hRatioIn->SetLineColor(kRed+1);
  hRatioIn->SetMarkerStyle(20);
  hRatioIn->SetMarkerColor(kRed+1);
  hRatioIn->Draw("same");
