...
void draw_planes() {
  ...
  TH1F* h_in_inclusive_small_raw = dynamic_cast<TH1F*>(fs->Get("hv_0"));
  TH1F* h_out_inclusive_small_raw = dynamic_cast<TH1F*>(fs->Get("hv_1"));

  std::unique_ptr<TFile> fl = std::unique_ptr<TFile>(new TFile("root_files/SeparatePtPlane_q2_70_100.root","READ"));
  TH1F* h_in_inclusive_large_raw = dynamic_cast<TH1F*>(fl->Get("hv_0"));
  TH1F* h_out_inclusive_large_raw = dynamic_cast<TH1F*>(fl->Get("hv_1"));
  ...
  int n = 16;
  Double_t bin_edges[17]={0,5,10,15,20,25,30,40,50,60,70,80,90,100,120,160,200};
  TH1* h_in_inclusive_large = h_in_inclusive_large_raw->Rebin(n, "h_in_inclusive_largeRe", bin_edges);
  TH1* h_in_inclusive_small = h_in_inclusive_small_raw->Rebin(n, "h_in_inclusive_smallRe", bin_edges);
  TH1* h_out_inclusive_large = h_out_inclusive_large_raw->Rebin(n, "h_out_inclusive_largeRe", bin_edges);
  TH1* h_out_inclusive_small = h_out_inclusive_small_raw->Rebin(n, "h_out_inclusive_smallRe", bin_edges);
